...
void variometro(){
  ...
  if (beep_on) {             
    VarioBeep(0.2, -5.0);  // Ajuste de umbrales de sonido
  }
}
...
void VarioMSCalculation() {
  ...
  float stime = tim[maxsamples - samples];                                   // Establecer el tiempo de inicio para el cálculo

  for (int cc = maxsamples - samples; cc < maxsamples; cc++) {                  // Realizar cálculos
  ...
  // Calcular la velocidad vertical (vario)
...
void VarioBeep(float TreshUp, float TreshDown) {
  if ((tempo - beep) > Beep_period + 100) { // Control del tiempo entre beeps                         
    ...
    if (vario > TreshUp && vario < 15) {                                                              
      ...
      tone(buzzer, (500 + (250 * vario)), abs(Beep_period));  // Emitir beep con tono variable
    }                                             
    else if (vario < -0.15) {                       // Si desciende rápidamente
      Beep_period = 200;                                           
      tone(buzzer, 200, 400);  // Emitir beep rápido en caso de descenso
    }                                                          
  } 
